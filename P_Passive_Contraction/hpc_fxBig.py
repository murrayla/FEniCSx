...
def fx_(tnm, file, r, pct, s, depth):
    ...
    if tnm == "test":
        ...
        azi.x.array[:], ele.x.array[:] = anistropic(tnm, r, azi_vals, ele_vals, x_n, r, depth)
    ...
    sph_file = io.VTXWriter(MPI.COMM_WORLD, os.path.dirname(os.path.abspath(__file__)) + f"/_SPH/_BIG_{r}.bp", ang, engine="BP4")
    sph_file.write(0)
    sph_file.close()
    ...
    #         F
    #     ) * df.inv(F.T)
    # s_piola = J * ufl.inv(F) * ufl.diff(Psi, F) * ufl.inv(F.T) - p * ufl.inv(G_v + 2 * E)
    ...
    for k in range(0, pct+1, 1):
        ...
        # Save CSV
        if s:
            csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "_csv", f"BIG_{int(r)}_{k}_stretch.csv")
        else:
            csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "_csv", f"BIG_{int(r)}_{k}.csv")
        df.to_csv(csv_path)  
...
def main(args):
    depth = 1
    # += Run
    print("\t" * depth + "!! BEGIN TEST: " + "BIG" + " !!")
    # += Enter FENICS
    file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "_msh/EMGEO_BIG_" + str(args.ref_level) + ".msh")
    fx_(args.test_num, file, args.ref_level, args.def_level, args.stretch_test, depth)
